...
1б. **Нов CSS → `app/static/css/`** като отделен файл, зареден с `<link rel="stylesheet" href="...">`. **Никога** нов CSS inline в `<style>` таг в `.html` файл — точно същото правило като за JS (правило 1), не по-ниско ниво на дисциплина. Малки, наистина УНИКАЛНИ (появяват се на точно 1 файл) inline `style="..."` атрибути на отделен елемент са ОК — щом един и същ CSS блок/правило се появи на 2-ри файл, важи правило 1а: спри и extract-вай.
1в. **ФОРМАЛЕН ПРИНЦИП (изрично поискан от потребителя, важи за JS И CSS еднакво):** `.html` темплейт файловете трябва да **ВИКАТ** логика (JS чрез `<script src>`, CSS чрез `<link rel="stylesheet">`), никога да не я **СЪДЪРЖАТ** директно. Изключения, позволени да останат inline: (а) малки Jinja data-инжекции (`window.X_DATA = {{ ... }}`) — носят сървърни данни, не логика; (б) уникално, наистина еднократно CSS/JS, което не се повтаря на 2-ри файл. Всичко друго → отделен файл.
