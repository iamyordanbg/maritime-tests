...
- **CSS theme override-ите (Light/Sepia/Ink цветовете) са дублирани в 3 темплейта** (`simulator.html`, `test.html`, `result_review.html`) — inline `<style>` блокове, почти идентични, никога извадени в общ `.css` файл. Точно нарушението, описано в новото правило 1а по-горе (архитектурни правила) — открито, но **не е поправено** до края на тази сесия. Следваща сесия: extract-вай в `app/static/css/reading-theme.css`, зареден с `<link>` във трите темплейта, следвайки СЪЩИЯ подход като `reading-prefs.js` за JS-а.
